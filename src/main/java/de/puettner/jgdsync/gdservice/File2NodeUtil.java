...
public class File2NodeUtil {
    ...
    public static Node<SyncData> fileList2NodeList(SyncNodeFactory syncNodeFactory, FileList fileList, Node<SyncData> parentNode) {
        ...
        for (File file : fileList.getFiles()) {
            if (DriveFileUtil.isFolder(file)) {
                Node<SyncData> node = new Node<>(syncNodeFactory.construct(false, new GDFile(file), null), true);
                parentNode.addChild(node);
            } else {
                parentNode.addChild(syncNodeFactory.construct(false, new GDFile(file), null));
            }
        }
        return parentNode;
    }
    ...
    public static Node<SyncData> file2Node(SyncNodeFactory syncNodeFactory, File file) {
        if (file == null)
            return null;
        return new Node<>(syncNodeFactory.construct(true, new GDFile(file), null), DriveFileUtil.isFolder(file));
    }
}
